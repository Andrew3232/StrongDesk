                            ...
                            <div class="col-xs-12 col-md-8 col-lg-6">
                                <div class="fn_step-2">
                                    <div class="input-group input-group--dabbl">
                                        <span class="input-group-addon input-group-addon--left">ID</span>
                                        <span class="fn_meta_field form-control fn_disabled" style="max-width: 60px">{$product->id|escape}</span>
...
<div class="okay_list_body_item variants_list_item fn_new_row_variant hidden">
    <div class="okay_list_row ">
        ...
        <div class="okay_list_boding okay_list_close remove_variant">
            <button data-hint="{$btr->general_delete_product|escape}" data-id="" type="button" class="btn_close fn_remove_variant hint-bottom-right-t-info-s-small-mobile  hint-anim">
                {include file='svg_icon.tpl' svgId='delete'}
            </button>
        </div>
    </div>
...
<script>
    $(window).on("load", function() {
        ...
        // Обновить/сохранить вариант
        $(document).on("click",".fn_variant_save",function () {
          let $this = $(this),
              product_id = $this.data('product_id'),
              parent = $this.closest('.variants_list_item');
          // console.log(parent.find('input, select').serializeArray());
            if(product_id){
              $.ajax({
                url: "ajax/update_variant.php",
                data: {
                  product_id: product_id,
                  variant: {
                    id: parent.find('[name="variants[id][]"]').val(),
                    sku: parent.find('[name="variants[sku][]"]').val(),
                    name: parent.find('[name="variants[name][]"]').val(),
                    price: parent.find('[name="variants[price][]"]').val(),
                    compare_price: parent.find('[name="variants[compare_price][]"]').val(),
                    currency_id: parent.find('[name="variants[currency_id][]"]').val(),
                    weight: parent.find('[name="variants[weight][]"]').val(),
                    stock: parent.find('[name="variants[stock][]"]').val(),
                    units: parent.find('[name="variants[units][]"]').val(),
                    material: parent.find('[name="variants[material][]"]').val(),
                    shape: parent.find('[name="variants[shape][]"]').val(),
                    size: parent.find('[name="variants[size][]"]').val(),
                    color: parent.find('[name="variants[color][]"]').val(),
                  }
                },
                dataType: 'json',
                success: function(data){
