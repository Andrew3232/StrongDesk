...
<div class="fn_product product_wrap block" itemscope itemtype="http://schema.org/Product">
    ...
    <div class="fn_transfer f_row flex-column flex-lg-row align-items-lg-stretch product_inner">
        <div class="block f_col-xl-6 f_col-lg-6 f_col-md-11 f_col-sm-12 f_col-12">
                        ...
                        <div class="product-page__img swiper-container gallery-top">
                            <div class="swiper-wrapper">
                                {foreach $product->images as $i=>$image}
                                    <a href="{$image->filename|resize:1800:1800:w}" data-fancybox="we2"
                                       class="gallery-top__image swiper-slide _ibg _ibg--contain">
										<span class="gallery-top__zoom_icon fn_button_zoom_product"
                                              title="{$lang->email_comment_look}">{include file="svg.tpl" svgId="compress_icon"}</span>
                                        <picture>
                                            {if $settings->support_webp}
                                            <source type="image/webp" srcset="{$image->filename|resize:700:800|webp}>
                                            {/if}
                                                <source srcset=" {$image->filename|resize:700:800}">
                                            <img {if $image@first} itemprop="image" {/if} src="{$image->filename|resize:700:800}"
                                        ...
                                        <picture>
                                            {if $settings->support_webp}
                                                <source type="image/webp"
                                                        data-srcset="{$image->filename|resize:170:170|webp}">
                                            {/if}
                                            <source data-srcset="{$image->filename|resize:170:170}">
                                            <img class="lazy"
                                                 data-src="{$image->filename|resize:170:170}"
        ...
        <div class="block product-page__details f_col-xl-5 f_col-lg-6 f_col-md-11 f_col-sm-12 f_col-12">
            <div class="boxed--stretch details_boxed">
                <div class="details_boxed__item">
                    {* The product name *}
                    <div class="details_boxed__header header_product {if $product->variant->sku}block__header--promo{/if}">
                        <div
                                class="block__header_promo header_product__sku product-page__sku{if !$product->variant->sku} hidden{/if}">
                ...
                <div class="details_boxed__item details_boxed__elements elements_product">
                    {* Product Rating *}
                    <div class="elements_product__column">
                        ...
                        <div class="details_boxed__available">
                            <div
                                    class="available__no_stock d-flex align-items-center icon icon-highlight-off fn_not_stock{if $product->variant->stock > 0} hidden-xs-up{/if}"
                                    data-language="product_out_of_stock">{$lang->product_out_of_stock}</div>
                            <div
                                    class="available__in_stock d-flex align-items-center icon icon-check-circle-outline fn_in_stock{if $product->variant->stock < 1} hidden-xs-up{/if}"
                ...
                <div class="details_boxed__item">
                    <form class="fn_variants product_detail_page" action="{url_generator route="cart"}">

                        {* Product variants *}
                        <div class="details_boxed__select {if $product->variants|count < 2} hidden{/if}">
                            <div class="details_boxed__title"
                                 data-language="product_variant">{$lang->product_variant}:
                            </div>
                            <select name="variant"
                                ...
                                {foreach $product->variants as $v}
                                    <option{if $product->variant->id == $v->id} selected{/if}
                                            ...
                                            data-material="{if $v->material}{$v->material}{else}0{/if}"
                                            data-size="{if $v->size}{$v->size}{else}0{/if}"
                                            data-color="{if $v->color}{$v->color}{else}0{/if}"
                                            data-shape="{if $v->shape}{$v->shape}{else}0{/if}"
                                    >
                                        {if $v->size}size={$v->size} color={$v->color}material {$v->material}shape={$v->shape}{else}{$product->name|escape}{/if}</option>
                                {/foreach}
                            </select>
                            <div class="dropDownSelect2"></div>
                        </div>

                        <div class="details_boxed__offer offer_product" itemprop="offers" itemscope=""
                            ...
                            <div class="d-flex align-items-end details_boxed__prices">
                                ...
                                <div
                                    ...
                                    <div
                                            class="fn_discount_label details_boxed_pct{if $product->variant->price>0 && $product->variant->compare_price>0 && $product->variant->compare_price>$product->variant->price}{else} hidden-xs-up{/if}">
                            ...
                            <div class="offer_product__actions">
                                {* Quantity *}
                                <div class="offer_product__amount fn_is_stock{if $product->variant->stock < 1} hidden{/if}">
                                    <div class="fn_product_amount  amount">
										<span class="fn_minus amount__minus">
											<span class="amount__icon">{include file="svg.tpl" svgId="minus_circle"}</span>
										</span>
                                        <input class="amount__input" type="text" name="amount" value="1"
                                               data-max="{$product->variant->stock}">
                                        <span class="fn_plus amount__plus">
											<span class="amount__icon">{include file="svg.tpl" svgId="plus_circle"}</span>
										</span>
                                    </div>
                                </div>

                                <div class="offer_product__button_inner">
                                    ...
                                    {else}
                                        ...
                                        <div class="fn_is_preorder {if $product->variant->stock > 0} hidden-xs-up{/if}">
                                            <button class="product-page__button product-page__button--preloader button"
                                                    type="submit"
                                    ...
                                    <div class="fn_is_stock {if $product->variant->stock < 1} hidden-xs-up{/if}">
                                        <button class="product-page__button button button--red" type="submit"
                                                data-language="product_add_cart">{$lang->product_add_cart} +
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex align-items-center details_boxed__buttons">
                                ...
                                {if is_array($comparison->ids) && in_array($product->id, $comparison->ids)}
                                    <a class="fn_comparison product-page__compare selected"
                                       href="#"
                                       data-id="{$product->id}"
                                       title="{$lang->remove_comparison}"
                                       data-result-text="{$lang->product_add_comparison}"
                                       data-language="product_remove_comparison">
                                        <i class="fa fa-balance-scale"></i>
                                    </a>
                                {else}
                                    <a class="fn_comparison product-page__compare"
                                       href="#"
                                       data-id="{$product->id}"
...
{*            <div class="details_boxed__item">*}
{*                <div class="details_boxed__categories categories_product">*}
{*                    <div class="categories_product__title">*}
{*                        <span data-language="features_catalog">{$lang->features_catalog} : </span>*}
{*                    </div>*}
{*                    {foreach $categories as $c}*}
{*                        <a class="categories_product__link"*}
{*                           href="{url_generator route="category" url=$c->url keyword=$keyword}">*}
{*                            {$c->name}*}
{*                            <span>,</span>*}
{*                        </a>*}
{*                    {/foreach}*}
...
{*            </div>*}
            ...
            <div class="fn_accordion accordion details_boxed__item">
                <div class="details_boxed__item">
                    {* Delivery *}
    ...
    <div class="tabs">
        <div class="tabs__navigation">
            ...
            <a id="fn_tab_comments" class="tabs__link" href="#comments">
                <span data-language="product_comments">{$lang->product_comments} ({(count($comments))})</span>
            </a>
        </div>
        ...
        <div class="tabs__content">
                                ...
                                <div class="d-flex justify-content-start features__wrap">
                                    ...
                                    <div class="features__value">
                                        {foreach $f->values as $value}
                                            ...
                                            {else}
                                                <span>{$value->value|escape}{if !$value@last},{/if}</span>
                                            {/if}
                                        {/foreach}
                                    </div>
                                </div>
            ...
            {* Comments *}
            <div id="comments" class="tab comments_wrap">
                <div class="comment-wrap">
                    <div class="comment">
                        {if $comments}
                            <div class="comment__label">
                                {count($comments)}
                                <span data-language="reviews_for">{$lang->reviews_for}</span>
                                <span class="comment__label_name">
								{$product->name}
							</span>
                            </div>
                                    ...
                                    <div class="comment__item {if $level > 0} admin_note{/if}">
                                        ...
                                        <div class="comment__inner">
                                            ...
                                            <div class="comment__boxed">
                                                <div class="d-flex flex-wrap align-items-start justify-content-between comment__header">
                                                    {* Comment name *}
                        ...
                        <form id="captcha_id" class="form fn_validate_product" method="post">
                            ...
                            <div class="form__body">
                                ...
                                <div class="form__group">
									<span class="form__group_label data-language="
                                          form_enter_comment">{$lang->form_enter_comment}<span>*</span></span>
                                ...
                                <div class="form__group_inner">
                                    {* User's name *}
                                    <div class="form__group">
													<span class="form__group_label"
                                                          data-language="form_name">{$lang->form_name}<span>*</span></span>
                                    ...
                                    <div class="form__group">
                                        <span class="form__group_label"
                                              data-language="form_email">{$lang->form_email}</span>
                            ...
                            <div class="form__footer">
                                ...
                                {* Submit button *}
                                <input class="form__button button button--red g-recaptcha"
                                       type="submit"
                                       name="comment"
                                       data-language="form_send" {if $settings->captcha_type == "invisible"}
                                    data-sitekey="{$settings->public_recaptcha_invisible}" data-badge='bottomleft'
                                    data-callback="onSubmit" {/if}
                                       value="{$lang->form_send}"
                                       title="{$lang->form_send}"/>
                            </div>
                        </form>
                    </div>
                </div>
...
"seller": {
"@type": "Organization",
"name": "{/literal}{$settings->site_name|escape}{literal}"                                                                                     {/literal}
*}