...
<div class="sidebar__boxed">
	<div class="fn_switch sidebar_heading d-flex align-items-center justify-content-between">
		...
		<span class="d-flex align-items-center filter__name_arrow sidebar_heading_arrow  fa fa-chevron-down"></span>
	</div>
						...
						{if $c->visible && ($c->has_posts || $settings->show_empty_categories)}
							{if $c->subcategories && $c->count_children_visible && $level < 3}
								<li class="blog_catalog__item parent">
									<a class="blog_catalog__link {if $category->id == $c->id} selected{/if}"
										href="{url_generator route='blog_category' url=$c->url}" data-blog_category="{$c->id}">
										{if $c->image}
											<picture>
												{if $settings->support_webp}
													<source type="image/webp"
														data-srcset="{$c->image|resize:20:20:false:$config->resized_blog_categories_dir|webp}">
												{/if}
												<source data-srcset="{$c->image|resize:20:20:false:$config->resized_blog_categories_dir}">
												<img class="lazy" data-src="{$c->image|resize:20:20:false:$config->resized_blog_categories_dir}"
													src="{$rootUrl}/design/{get_theme}/images/xloading.gif" alt="{$c->name|escape}"
													title="{$c->name|escape}" />
											</picture>
										{else}
											<span class="blog_catalog__no_image d-flex align-items-center justify-content-center"
												title="{$c->name|escape}">
									...
									<span class="fn_switch blog_catalog__switch">{include file='svg.tpl' svgId='arrow_right'}</span>
...
{if !empty($post->categories)}
	<div class="sidebar__boxed">
		<div class="fn_switch sidebar_heading d-flex align-items-center justify-content-between">
			<span data-language="blog_tags">{$lang->blog_tags}</span>
			<span class="d-flex align-items-center filter__name_arrow sidebar_heading_arrow  fa fa-chevron-down"></span>
		</div>
	...
	<div class="sidebar_subscribe">
		...
		<form class="sidebar_subscribe__form fn_validate_subscribe" method="post">
			<div class="sidebar_subscribe__group">
				...
				<input class="form__input form__input--aside_subscribe" aria-label="subscribe" type="email"
					name="subscribe_email" value="" data-format="email" placeholder="{$lang->form_email}" />
			</div>
			<button class="button button--basic button--aside_subscribe" type="submit"
				title="{$lang->subscribe_button}">{include file='svg.tpl' svgId='subscribe_image'}</button>
		</form>
	</div>
</div>
			...
			<div class="sidebar_card f_row">
				{foreach $featured_products as $product}
					<div class="sidebar_card__item f_col-12">
						<a class="d-flex align-items-center justify-content-center sidebar_card__link"
							...
							<div class="sidebar_card__image">
								{if $product->image->filename}
									<picture>
										...
										<img class="lazy" data-src="{$product->image->filename|resize:60:60}"
											src="{$rootUrl}/design/{get_theme}/images/xloading.gif" alt="{$product->name|escape}"
											title="{$product->name|escape}" />
									</picture>
								{else}
									<div class="sidebar_card__no_image d-flex align-items-center justify-content-center"
										title="{$product->name|escape}">
										{include file="svg.tpl" svgId="no_image"}
									</div>
								{/if}
				...
				{/foreach}
				<a class="sidebar_card__more d-flex align-items-center f_col-12"
