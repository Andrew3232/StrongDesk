	...
	<div class="post_container post_container--left">
		<div class="post_container__wrapper">
			<div class="post_container__wallpaper"
				style="background: #F4F6F9 url({$post->image|resize:1100:600:false:$config->resized_blog_dir}) no-repeat center center;background-size: cover;">
				{* Post category label *}
			...
			<div class="post_container__boxed">
				<div class="post_container__header">
					...
					<div class="post_information">
						...
						{* Reading time *}
						{if $post->read_time > 0}
							<div class="post_information__item"
								title="{$lang->blog_time_read} {if $post->read_time > 0}{$post->read_time} {$post->read_time|plural:$lang->blog_time_read_minute_1:$lang->blog_time_read_minute_2:$lang->blog_time_read_minute_3}{else}{$lang->blog_unknown_time_read}{/if}">
								{include file="svg.tpl" svgId="time_read_icon"}
								<span>{$lang->blog_time_read} {$post->read_time}
									{$post->read_time|plural:$lang->blog_time_read_minute_1:$lang->blog_time_read_minute_2:$lang->blog_time_read_minute_3}</span>
							</div>
						{/if}
						{* Counts of comments *}
					...
					{if $post->updated_date > 0}
						<div class="post__update_date">
							...
							<span class="post__update_date_text">{$lang->blog_update_date}
								{$post->updated_date|date:"d m Y"}</span>
						</div>
					{/if}
					{* Table contents *}
				...
				<div class="post_container__footer">
					...
					<div class="post_share">
						...
						<div id="post_{$post->id}" class="post__rating product__rating fn_rating"
							data-rating_post_url="{url_generator route='ajax_post_rating'}">
							<div class="share__text post_share__text">
					...
					{if $post->author}
						<div class="post_author">
							<div class="post_author__images">
								<div class="post_author__img _ibg">
									{if $post->author->image}
										<img
											src="{$post->author->image|resize:200:200:false:$config->resized_authors_dir:center:top}"
											alt="{$post->author->name|escape}">
									{else}
										{include file="svg.tpl" svgId="avatar_icon"}
									{/if}
								</div>
							...
							<div class="post_author__infobox">
								...
								{if $post->author->visible}
									<a href="{url_generator route='author' url=$post->author->url}" class="post_author__btn">
										<span data-language="blog_author_all_posts">{$lang->blog_author_all_posts}</span>
									</a>
								{/if}
								...
								{if is_array($post->author->socials)}
									<div class="post_author__social">
										{foreach $post->author->socials as $social}
											<a class="fn_social_image social__link {$social.domain|escape}" rel="noreferrer"
												aria-label="{$social_domain}"
		...
		<div class="block">
			...
			<div id="comments">
				<div class="comment-wrap ">
					...
					<div class="form_wrap ">
						{* Comment form *}
						<form id="fn_blog_comment" class="fn_validate_post form" method="post" action="">
							...
							<div class="form__body">
								...
								<div class="form__group_inner">
									{* User's name *}
									<div class="form__group">
										<span class="form__group_label" data-language="form_name">{$lang->form_name}<span>*</span></span>
									...
									{* User's email *}
									<div class="form__group">
										<span class="form__group_label" data-language="form_email">{$lang->form_email}</span>
							...
							<div class="form__footer">
								...
								<div class="captcha">
									<div class="secret_number">{$captcha_comment[0]|escape} + ? = {$captcha_comment[1]|escape}
									</div>
								...
								{* Submit button *}
								<button class="form__button button button--red g-recaptcha" type="submit" name="comment"
